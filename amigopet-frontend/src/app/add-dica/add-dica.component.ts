...
export class AddDicaComponent implements OnInit {
  constructor(
    private dicaServico: DicaService,
    private usuarioServico: TokenServiceService
  ) {}
  ...
  idUsuer: string = '0';
  ngOnInit(): void {
    this.usuarioServico.getUser().subscribe((resolve) => {
  ...
  getDicasByIdUser() {
    ...
    this.dicas.subscribe((resp) => console.log(resp));
  }

  excluirDica(id?: number) {
    this.dicaServico.excluirDica(id).subscribe((resolve) => {
